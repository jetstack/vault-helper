...
func init() {
	...
	readCmd.PersistentFlags().String(read.FlagOwner, "0", "Set owner of output file. Uid value also accepted. (default <current user>)")
	readCmd.Flag(read.FlagOwner).Shorthand = "o"
	readCmd.PersistentFlags().String(read.FlagGroup, "0", "Set group of output file. Gid value also accepted. (default <current user-group>)")
