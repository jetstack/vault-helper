...
func (c *Cert) Run(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("error parsing %s [int] '%d': %v", FlagKeyBitSize, vInt, err)
	}
	c.SetBitSize(vInt)

	vStr, err := cmd.PersistentFlags().GetString(FlagKeyType)
	if err != nil {
		return fmt.Errorf("error parsing %s [string] '%s': %v", FlagKeyType, vStr, err)
	}
	c.SetKeyType(vStr)

	vStr, err = cmd.PersistentFlags().GetString(FlagOwner)
	if err != nil {
		return fmt.Errorf("error parsing %s [string] '%s': %v", FlagOwner, vStr, err)
	}
	c.SetOwner(vStr)

	vStr, err = cmd.PersistentFlags().GetString(FlagGroup)
	if err != nil {
		return fmt.Errorf("error parsing %s [string] '%s': %v", FlagGroup, vStr, err)
	}
	c.SetGroup(vStr)

	vSli, err := cmd.PersistentFlags().GetStringSlice(FlagIpSans)
	if err != nil {
		return fmt.Errorf("error parsing %s [[]string] '%s': %v", FlagIpSans, vSli, err)
	}
	c.SetIPSans(vSli)
